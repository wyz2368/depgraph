...
def get_values_even_if_ineligible(env_short_name, new_epoch, is_defender, save_count):
    '''
    ...
    Return the integer in {0, . . ., save_count} of the best.
    '''
    ...
    is_eligible = any(is_eligible)
    return vs_eq_gains, vs_retrain_gains, winner_index, is_eligible
...
def get_values(env_short_name, new_epoch, is_defender, save_count):
    ...
    '''
    vs_eq_gains, vs_retrain_gains, winner_index, is_eligible = \
        get_values_even_if_ineligible(env_short_name, new_epoch, is_defender, save_count)
    if not is_eligible:
        return None
