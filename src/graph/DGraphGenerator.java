...
public final class DGraphGenerator {
	...
	public static void genSepLayGraph(
		final DependencyGraph dag, final RandomDataGenerator rand
		...
		final double maxPosInactiveProb // random
		, final double aNodeCostFactor, final double aEdgeCostFactor, 
		...
		for (int i = 0; i < nodeList.length; i++) {
			...
			if (depGraph.inDegreeOf(node) == 0) {
				...
				layer[node.getId() - 1] =
					layer[edgeList.get(0).getsource().getId() - 1] + 1;
			}
		}
		for (Node node : depGraph.vertexSet()) {
			...
			genAlertProbRandom(node, rand, minPosActiveProb, 
				maxPosActiveProb, minPosInactiveProb, maxPosInactiveProb);
		}
		for (Edge edge : depGraph.edgeSet()) {
			...
			if (edge.gettarget().getActivationType() == NodeActivationType.OR) {
				...
				genActivationProbRandom(
					edge, rand, aEdgeActProbLB, aEdgeActProbUB);
			}
		}
	}

	public static void genGraph(final DependencyGraph dag,
		...
		, final double aRewardFactor, final double dPenaltyFactor) { 
			...
			for (int i = 0; i < depGraph.vertexSet().size(); i++) {
				...
				if (edgeSet != null && !edgeSet.isEmpty()) { // non-root nodes
					if (node.getActivationType() == NodeActivationType.OR) {
						shortestPathLength[node.getId() - 1] =
							Integer.MAX_VALUE;
						for (Edge edge : edgeSet) {
							shortestPathLength[node.getId() - 1] =
								Math.min(shortestPathLength[node.getId() - 1]
								, shortestPathLength[
				                     edge.getsource().getId() - 1]);
						}
					} else {
						for (Edge edge : edgeSet) {
							shortestPathLength[node.getId() - 1] =
								Math.max(shortestPathLength[node.getId() - 1]
								, shortestPathLength[
				                     edge.getsource().getId() - 1]);
						}
					}
			...
			for (Node node : depGraph.vertexSet()) {
				...
				genActivationProbRandom(
					node, rand, aNodeActProbLB, aNodeActProbUB);
				if (node.getType() != NodeType.TARGET) {
					node.setAReward(0.0);
					node.setDPenalty(0.0);
				}
				...
				genAlertProbRandom(
					node, rand, minPosActiveProb, maxPosActiveProb,
	...
	public static void genGraph(final DependencyGraph dag, 
		...
		, final double minPosInactiveProb, final double maxPosInactiveProb) {
		if (dag == null || rand == null || numTarget < 0 
			|| !isProb(nodeActTypeRatio)
			|| aRewardLB > aRewardUB || dPenaltyLB > dPenaltyUB
			|| aNodeCostLB > aNodeActProbUB
			|| aEdgeActProbLB > aEdgeActProbUB || dCostLB > dCostUB
			|| !isProb(aNodeActProbLB) || !isProb(aNodeActProbUB)
			|| aNodeActProbLB > aNodeActProbUB
			|| !isProb(aEdgeActProbLB) || !isProb(aEdgeActProbUB)
			|| aEdgeActProbLB > aEdgeActProbUB 
			|| !isProb(minPosActiveProb) || !isProb(maxPosActiveProb)
			|| minPosActiveProb > maxPosActiveProb
			|| !isProb(minPosInactiveProb) || !isProb(maxPosInactiveProb)
			|| minPosInactiveProb > maxPosInactiveProb
		) {
		...
		for (Node node : depGraph.vertexSet()) {
			setNodeTypeRandom(depGraph, node, rand, nodeActTypeRatio);
			genNodePayoffRandom(
				node, rand, aRewardLB, aRewardUB, dPenaltyLB,
				dPenaltyUB, aNodeCostLB, aNodeCostUB, dCostLB, dCostUB);
			genActivationProbRandom(node, rand, aNodeActProbLB, aNodeActProbUB);
			if (node.getType() != NodeType.TARGET) {
				node.setAReward(0.0);
				node.setDPenalty(0.0);
			...
			genAlertProbRandom(node, rand, minPosActiveProb, maxPosActiveProb,
				minPosInactiveProb, maxPosInactiveProb);
		}
		for (Edge edge : depGraph.edgeSet()) {
			if (edge.gettarget().getActivationType() == NodeActivationType.OR) {
				genEdgePayoffRandom(edge, rand, aEdgeCostLB, aEdgeCostUB);
				genActivationProbRandom(edge, rand,
					aEdgeActProbLB, aEdgeActProbUB);
			}
		}
	}
	...
	public static void findMinCut(final DependencyGraph depGraph) {
		...
		SimpleDirectedWeightedGraph<Node, Edge> cloneGraph =
			new SimpleDirectedWeightedGraph<Node, Edge>(Edge.class);
        for (Node node : depGraph.vertexSet()) {
        ...
        for (Node node : cloneGraph.vertexSet()) {
        	if (node.getTopoPosition() != -1
    			&& cloneGraph.inDegreeOf(node) == 0) {
        		Edge newEdge = new Edge();
        ...
        EdmondsKarpMFImpl<Node, Edge> minCutAlgo =
    		new EdmondsKarpMFImpl<Node, Edge>(cloneGraph);
	...
	private static void setNodeTypeRandom(
		...
		if (depGraph == null || node == null
			|| rand == null || !isProb(typePivot)) {
			throw new IllegalArgumentException();
		}
	...
	private static void genAlertProbRandom(
		...
		final double posActiveProb =
			safeUniform(minPosActiveProb, maxPosActiveProb, rand);
		final double posInactiveProb =
			safeUniform(minPosInactiveProb, maxPosInactiveProb, rand);
	...
	private static void genNodePayoffRandom(
		...
		if (
			node == null || rand == null
			|| aRewardLB > aRewardUB || dPenaltyLB > dPenaltyUB
			|| aCostLB > aCostUB || dCostLB > dCostUB
		) {
			throw new IllegalArgumentException();
		}
	...
	private static void genEdgePayoffRandom(
		final Edge edge, final RandomDataGenerator rand
		, final double aCostLB, final double aCostUB) {
		double aCost = safeUniform(aCostLB, aCostUB, rand);
		edge.setACost(aCost);
	}
	
	private static void genActivationProbRandom(
		final Node node, final RandomDataGenerator rand
	...
	private static void setTopologicalOrder(
		final DependencyGraph depGraph) {
