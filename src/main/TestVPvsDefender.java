...
// "/Users/thanhnguyen/Documents/WORKS/ATTACK_GRAPH/EXPERIMENTS/"
		// + "E2_SepLay/2_GraphGameSim_SepLay/graphs";
//		int graphID = 1;
...
//		DependencyGraph depGraph = DGraphUtils.loadGraph(filePathName);
		...
		Defender valuePropagationvsDefender = new ValuePropagationVsDefender(
		 ...
		 Defender randomWalkvsDefender =
			 new RandomWalkVsDefender(
				 logisParam, discFact, thres, qrParam, numRWSample, 1.0);
		
		Attacker vpAttacker = new ValuePropagationAttacker(
			maxNumSelectCandidate, minNumSelectCandidate
			, numSelectCandidateRatio, qrParam, discFact, 0.0);
		
		 Attacker rwAttacker =
			 new RandomWalkAttacker(numRWSample, qrParam, discFact);
				
		final double thousand = 1000.0;
		
		long start = System.currentTimeMillis();
		GameSimulation gameSimVPvsVP =
			new GameSimulation(depGraph, vpAttacker, valuePropagationvsDefender,
				rnd, numTimeStep, discFact);
		...
		for (int i = 0; i < numSim; i++) {
			...
			defPayoffVPvsVP +=
				gameSimVPvsVP.getSimulationResult().getDefPayoff();
			attPayoffVPvsVP +=
				gameSimVPvsVP.getSimulationResult().getAttPayoff();
			
			defPayoffStdVPvsVP +=
				Math.pow(gameSimVPvsVP.getSimulationResult().getDefPayoff(), 2);
			attPayoffStdVPvsVP +=
				Math.pow(gameSimVPvsVP.getSimulationResult().getAttPayoff(), 2);
			gameSimVPvsVP.reset();
		}
		...
		defPayoffStdVPvsVP = Math.pow(
			defPayoffStdVPvsVP / numSim - Math.pow(defPayoffVPvsVP, 2), half);
		attPayoffStdVPvsVP = Math.pow(
			attPayoffStdVPvsVP / numSim - Math.pow(attPayoffVPvsVP, 2), half);
		
		timeVPvsVP = (end - start) / thousand / numSim;
		
		GameSimulation gameSimVPvsRW =
				new GameSimulation(depGraph, vpAttacker, randomWalkvsDefender,
					rnd, numTimeStep, discFact);
		...
		for (int i = 0; i < numSim; i++) {
			...
			defPayoffVPvsRW +=
				gameSimVPvsRW.getSimulationResult().getDefPayoff();
			attPayoffVPvsRW +=
				gameSimVPvsRW.getSimulationResult().getAttPayoff();
			
			defPayoffStdVPvsRW +=
				Math.pow(gameSimVPvsRW.getSimulationResult().getDefPayoff(), 2);
			attPayoffStdVPvsRW +=
				Math.pow(gameSimVPvsRW.getSimulationResult().getAttPayoff(), 2);
		...
		defPayoffStdVPvsRW = Math.pow(
			defPayoffStdVPvsRW / numSim - Math.pow(defPayoffVPvsRW, 2), half);
		attPayoffStdVPvsRW = Math.pow(
			attPayoffStdVPvsRW / numSim - Math.pow(attPayoffVPvsRW, 2), half);
		timeVPvsRW = (end - start) / thousand / numSim;
		
		start = System.currentTimeMillis();
		GameSimulation gameSimRWvsVP =
			new GameSimulation(
				depGraph, rwAttacker, valuePropagationvsDefender,
			rnd, numTimeStep, discFact);
		...
		for (int i = 0; i < numSim; i++) {
			...
			defPayoffRWvsVP +=
				gameSimRWvsVP.getSimulationResult().getDefPayoff();
			attPayoffRWvsVP +=
				gameSimRWvsVP.getSimulationResult().getAttPayoff();
			
			defPayoffStdRWvsVP +=
				Math.pow(gameSimRWvsVP.getSimulationResult().getDefPayoff(), 2);
			attPayoffStdRWvsVP +=
				Math.pow(gameSimRWvsVP.getSimulationResult().getAttPayoff(), 2);
			gameSimRWvsVP.reset();
		}
		end = System.currentTimeMillis();
		defPayoffRWvsVP /= numSim;
		attPayoffRWvsVP /= numSim;
		defPayoffStdRWvsVP = Math.pow(
			defPayoffStdRWvsVP / numSim - Math.pow(defPayoffRWvsVP, 2), half);
		attPayoffStdRWvsVP = Math.pow(
			attPayoffStdRWvsVP / numSim - Math.pow(attPayoffRWvsVP, 2), half);
		timeRWvsVP = (end - start) / thousand / numSim;
		
		GameSimulation gameSimRWvsRW =
				new GameSimulation(depGraph, rwAttacker, randomWalkvsDefender,
					rnd, numTimeStep, discFact);
		...
		for (int i = 0; i < numSim; i++) {
			...
			defPayoffRWvsRW +=
				gameSimRWvsRW.getSimulationResult().getDefPayoff();
			attPayoffRWvsRW +=
				gameSimRWvsRW.getSimulationResult().getAttPayoff();
			
			defPayoffStdRWvsRW +=
				Math.pow(gameSimRWvsRW.getSimulationResult().getDefPayoff(), 2);
			attPayoffStdRWvsRW +=
				Math.pow(gameSimRWvsRW.getSimulationResult().getAttPayoff(), 2);
			gameSimRWvsRW.reset();
		}
		end = System.currentTimeMillis();
		defPayoffRWvsRW /= numSim;
		attPayoffRWvsRW /= numSim;
		defPayoffStdRWvsRW = Math.pow(
			defPayoffStdRWvsRW / numSim - Math.pow(defPayoffRWvsRW, 2), half);
		attPayoffStdRWvsRW = Math.pow(
			attPayoffStdRWvsRW / numSim - Math.pow(attPayoffRWvsRW, 2), half);
		timeRWvsRW = (end - start) / thousand / numSim;

	
		System.out.println("Defender value propagation payoff: "
			+ defPayoffVPvsVP + "\t Std " + defPayoffStdVPvsVP);
		System.out.println("Attacker value propagation payoff: "
			+ attPayoffVPvsVP + "\t Std " + attPayoffStdVPvsVP);
		System.out.println("Runtime per simulation: " + timeVPvsVP);
		System.out.println();
		
		System.out.println("Defender random walk payoff: "
			+ defPayoffVPvsRW + "\t Std " + defPayoffStdVPvsRW);
		System.out.println("Attacker value propagation payoff: "
			+ attPayoffVPvsRW + "\t Std " + attPayoffStdVPvsRW);
		System.out.println("Runtime per simulation: " + timeVPvsRW);
		System.out.println();
		
		System.out.println("Defender value propagation payoff: "
			+ defPayoffRWvsVP + "\t Std " + defPayoffStdRWvsVP);
		System.out.println("Attacker random walk payoff: "
			+ attPayoffRWvsVP + "\t Std " + attPayoffStdRWvsVP);
		System.out.println("Runtime per simulation: " + timeRWvsVP);
		System.out.println();
		
		System.out.println("Defender random walk payoff: "
			+ defPayoffRWvsRW + "\t Std " + defPayoffStdRWvsRW);
		System.out.println("Attacker random walk payoff: "
			+ attPayoffRWvsRW + "\t Std " + attPayoffStdRWvsRW);
