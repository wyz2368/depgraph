...
public final class AgentFactory {
	...
	public static Attacker createAttacker(final String attackerName,
		...
		assert attackerName != null
			&& AttackerType.valueOf(attackerName) != null;
		...
		if (attType == AttackerType.UNIFORM) {
			if (!attackerParams.containsKey(
				AttackerParam.MAX_NUM_SELECT_CAND.toString())
				|| !attackerParams.containsKey(
					AttackerParam.MIN_NUM_SELECT_CAND.toString())
				|| !attackerParams.containsKey(
					AttackerParam.NUM_SELECT_CAND_RATIO.toString())
				|| !attackerParams.containsKey(
					AttackerParam.STDEV.toString())) {
				throw new IllegalArgumentException();
			}
			...
			|| !attackerParams.containsKey(
				AttackerParam.NUM_SELECT_CAND_RATIO.toString())
			|| !attackerParams.containsKey(
				AttackerParam.QR_PARAM.toString())
			|| !attackerParams.containsKey(
				AttackerParam.STDEV.toString())) {
				throw new IllegalArgumentException();
			}
			...
			if (!attackerParams.containsKey(
				AttackerParam.NUM_RW_SAMPLE.toString()) 
			...
			return new RandomWalkAttacker(
				attackerParams.get(AttackerParam.NUM_RW_SAMPLE.toString())
				, attackerParams.get(
					AttackerParam.QR_PARAM.toString()), discFact);
	...
	public static Defender createDefender(final String defenderName,
		...
		assert defenderName != null
			&& DefenderType.valueOf(defenderName) != null;
		...
		if (defType == DefenderType.UNIFORM) {
			...
			|| !defenderParams.containsKey(
				DefenderParam.maxNumAttCandidate.toString())
			|| !defenderParams.containsKey(
				DefenderParam.minNumAttCandidate.toString())
			|| !defenderParams.containsKey(
				DefenderParam.numAttCandidateRatio.toString())
			|| !defenderParams.containsKey(
				DefenderParam.numAttCandStdev.toString())
			|| !defenderParams.containsKey(DefenderParam.isTopo.toString())) {
				throw new IllegalArgumentException();
			}
			final int expectedKeys = 11;
			if (defenderParams.keySet().size() != expectedKeys) {
				throw new IllegalArgumentException();
			}
			return new ValuePropagationVsDefender(
				defenderParams.get(DefenderParam.maxNumRes.toString())
				, defenderParams.get(DefenderParam.minNumRes.toString())
				, defenderParams.get(DefenderParam.numResRatio.toString())
				, defenderParams.get(DefenderParam.logisParam.toString())
				...
				, defenderParams.get(DefenderParam.isTopo.toString()));
			...
			|| !defenderParams.containsKey(
				DefenderParam.isRandomized.toString())) {
				throw new IllegalArgumentException();
			}
			final int expectedKeys = 5;
			if (defenderParams.keySet().size() != expectedKeys) {
			...
			return new RandomWalkVsDefender(
				defenderParams.get(DefenderParam.logisParam.toString())
				, discFact
				, defenderParams.get(DefenderParam.bThres.toString())
				, defenderParams.get(DefenderParam.qrParam.toString())
