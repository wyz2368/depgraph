 ...
 */
public final class DepgraphPy4JNoNetCLIPort {
	 ...
	 */
	private static DepgraphPy4JNoNetCLIPort singleton;
	 ...
	 */
	private static final int MIN_PORT = 26433;
	
	/**
	 * Maximum allowed port number.
	 */
	private static final int MAX_PORT = 26833;
	 ...
	 */
	private DepgraphPy4JNoNetCLIPort(
	 ...
	 * @param args has 4 args: defStratName, attakcerStratName,
	 * graphFileName, portNum
	 */
	public static void main(final String[] args) {
		final int argsCount = 4;
		if (args == null || args.length != argsCount) {
			throw new IllegalArgumentException(
		"Need 4 args: defStratName, attackerStratName, graphFileName, portNum"
			);
		}
		...
		final int portNum = Integer.parseInt(args[3]);
		if (portNum < MIN_PORT || portNum > MAX_PORT || portNum % 2 == 0) {
			throw new IllegalArgumentException("Invalid port: " + portNum);
		}
		
		// set up Py4J server
		singleton = new DepgraphPy4JNoNetCLIPort(
			simSpecFolderName,
			defStratName,
			attackerStratName,
			graphFileName
		...
		final GatewayServer gatewayServer =
			new GatewayServer(singleton, portNum, portNum + 1, 0, 0, null);
		gatewayServer.start();
	...
	/**
	 * Get a new DepgraphPy4JNoNetCLIPort object for Py4J.
	 * @return the DepgraphPy4JNoNetCLIPort for Py4J to use.
	 */
	public static DepgraphPy4JNoNetCLIPort getGame() {
		return singleton;
	}
