...
def write_att_port(env_short_name, is_train, att_port):
    port_name = env_short_name + "_train_att_port.txt"
    if not is_train:
        port_name = env_short_name + "_eval_att_port.txt"
    with open(port_name, 'w') as file:
        file.write(str(att_port) + "\n")

def is_att_unlocked(env_short_name, is_train):
...
def lock_att(env_short_name, is_train):
    if not is_att_unlocked(env_short_name, is_train):
...
def wait_for_att_lock(env_short_name, is_train):
    sleep_time = 5
    while not is_att_unlocked(env_short_name, is_train):
...
def run_training(env_short_name, new_epoch, env_name_def_net, att_port):
    cmd_list = ["python3", "train_dg_java_mlp_att_vs_mixed.py", env_name_def_net, \
        env_short_name, str(new_epoch), str(att_port)]
...
def run_evaluation(env_short_name, new_epoch, env_name_def_net, att_port):
...
def main(graph_name, env_short_name, new_epoch, env_name_def_net, def_port):
    ...
    is_train = True
    wait_for_att_lock(env_short_name, is_train)
    lock_att(env_short_name, is_train)

    write_att_port(env_short_name, is_train, att_port)
    run_training(env_short_name, new_epoch, env_name_def_net, att_port)

    is_train = False
    wait_for_att_lock(env_short_name, is_train)
    lock_att(env_short_name, is_train)
    write_att_port(env_short_name, is_train, att_port)
    run_evaluation(env_short_name, new_epoch, env_name_def_net, att_port)
    close_env_process(env_process)

'''
example: python3 train_test_att.py SepLayerGraph0_noAnd_B.json sl29 16 \
    DepgraphJavaEnvVsMixedDef29N-v0 25333
'''
if __name__ == '__main__':
    if len(sys.argv) != 6:
        raise ValueError("Need 5 args: graph_name, env_short_name, new_epoch, " + \
            "env_name_def_net, def_port")
