'''
Run each of selected attacker and defender strategies against each other,
and return the mean payoff tuples in a Json object.
...
from cli_enjoy_dg_no_net import get_payoffs_no_net
...
def get_net_scope(net_name):
    ...
    if "epoch6" in net_name:
        return "deepq_train_e6"
    if "epoch7" in net_name:
        return "deepq_train_e7"
    if "epoch8" in net_name:
        return "deepq_train_e8"
    if "epoch9" in net_name:
        return "deepq_train_e9"
    if "epoch10" in net_name:
        return "deepq_train_e10"
    if "epoch11" in net_name:
        return "deepq_train_e11"
    return None
...
def get_result_dict(env_name_def_net, env_name_att_net, env_name_both, \
    ...
    '''
    Run each of def_heuristics and def_networks against each of att_heuristics and
    att_networks,
    ...
    for def_heuristic in def_heuristics:
        # run def_heuristic against all att_heuristics
        for att_heuristic in att_heuristics:
            mean_rewards_tuple = get_payoffs_no_net( \
                num_sims, def_heuristic, att_heuristic, graph_name)
            print(str((def_heuristic, att_heuristic)) + "\n" + str(mean_rewards_tuple))
            result[def_heuristic][att_heuristic] = list(mean_rewards_tuple)
