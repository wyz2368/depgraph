...
def main(env_name, env_short_name, new_epoch, retrain_number, att_port, \
    ...
    if is_vs_retrain:
        strat_file = env_short_name_tsv + "_epoch" + str(new_epoch) + "_retrain_def.tsv"
...
example: python3 enjoy_dg_data_vs_mixed_def_retraining.py DepgraphJavaEnvVsMixedDef29N-v0 \
    7 0 25335 s29 sl29_randNoAndB True
'''
if __name__ == '__main__':
    if len(sys.argv) != 9:
        raise ValueError("Need 8 args: env_name_def_net, env_short_name, new_epoch, " + \
            "retrain_number, att_port, port_lock_name, env_short_name_tsv, is_vs_retrain")
    ENV_NAME = sys.argv[1]
    ...
    main(ENV_NAME, ENV_SHORT_NAME, NEW_EPOCH, RETRAIN_NUMBER, ATT_PORT, PORT_LOCK_NAME, \
