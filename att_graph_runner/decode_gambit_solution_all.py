...
Takes as input the LCP solution from Gambit, and a 2-player .nfg (outcome version)
game file.
...
def get_all_mixture_weights(gambit_solution_string):
    '''
    Return the mixed strategy weights from the "LCP" solution method of Gambit.
    '''
    all_values = gambit_solution_string.splitlines()
    result = []
...
def print_all_outputs(attacker_strategies, defender_strategies, is_attacker_first, \
    all_mixture_weights):
    '''
    Print the corresponding strategy name with each mixture weight, for the given mixed
    strategy.
    '''
    if len(attacker_strategies) + len(defender_strategies) != len(all_mixture_weights[0]):
        raise ValueError("Wrong numbre of mixture weights: " + \
            str(len(all_mixture_weights[0])) + \
    ...
    for mixture_weights in all_mixture_weights:
...
def get_profile_names(gambit_string):
    '''
    Given the text of the game file, return the lists of attacker and defender
    strategy names in order, and whether the attacker is listed as the first player.
    '''
...
def main(game_file_name, solution_file_name):
    ...
    all_mixture_weights = get_all_mixture_weights(solution_data)
    print_all_outputs(attacker_strings, defender_strings, is_attacker_first, \
        all_mixture_weights)
