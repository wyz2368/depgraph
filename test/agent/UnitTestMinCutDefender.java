...
public final class UnitTestMinCutDefender {

	public UnitTestMinCutDefender() {
		// default constructor
	}
	...
	public void canRunTest() {
		final String simspecFolderName = "testDirs/simSpecMinCutDefPosStdev";
		...
		final int maxIdWithinMinCut = 3;
		for (final Node node: depGraph.vertexSet()) {
			if (node.getId() <= maxIdWithinMinCut) {
				depGraph.addMinCut(node);
			}
		}
		...
		final MinCutDefender testDefender =
			(MinCutDefender) AgentFactory.createDefender(defenderName, defenderParams, simSpec.getDiscFact());
