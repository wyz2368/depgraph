...
def print_outputs(attacker_strategies, defender_strategies, is_attacker_first, \
    ...
    with open(output_name, "w") as out_file:
        ...
        for i in range(len(defender_strategies)):
            ...
            if cur_weight > 0.0:
                out_file.write(cur_defender + "\t" + str(cur_weight) + "\n")
...
def decode(game_file_name, solution_file_name, output_name):
    '''
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        raise ValueError("Needs 3 arguments: game_file_name, solution_file_name, " + \
            "output_name")
    GAME_FILE_NAME = sys.argv[1]
    SOLUTION_FILE_NAME = sys.argv[2]
    OUTPUT_NAME = sys.argv[3]
    decode(GAME_FILE_NAME, SOLUTION_FILE_NAME, OUTPUT_NAME)
