...
def main(env_name_def_net, env_name_att_net, env_name_both, \
    ...
    out_file_name = "out_attPayoffs_" + str(env_short_name) + "_epoch" + str(new_epoch) + ".txt"
    if env_short_name is None:
        out_file_name = "out_attPayoffs_epoch" + str(new_epoch) + ".txt"
    if os.path.isfile(out_file_name):
        print("Skipping: " + out_file_name + " already exists.")
        return

    defender_mixed_strat = str(env_short_name) + "_epoch" + str(new_epoch) + "_def.tsv"
    if env_short_name is None:
        defender_mixed_strat = "d30_epoch" + str(new_epoch) + "_def.tsv"
    ...
    attacker_networks = "attNetStrings_" + str(env_short_name) + ".txt"
    if env_short_name is None:
        attacker_networks = "attNetStrings.txt"
